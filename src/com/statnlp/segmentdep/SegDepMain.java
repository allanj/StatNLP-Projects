package com.statnlp.segmentdep;
...
//		testFile = "data/semeval10t1/small.txt";
		...
		if(dataTypeSet.contains(DPConfig.dataType)){
			trainingInsts = SegDepReader.readCNN(trainingPath, true, trainNumber, trans);
			testingInsts = SegDepReader.readCNN(testFile, false,testNumber,trans);
		}else{
			trainingInsts = SegDepReader.readInstance(trainingPath, true,trainNumber,entities,trans);
			testingInsts = isPipe? SegDepReader.readFromPipeline(testFile,testNumber,trans, topKinput): SegDepReader.readInstance(testFile, false,testNumber,entities,trans);
		}
