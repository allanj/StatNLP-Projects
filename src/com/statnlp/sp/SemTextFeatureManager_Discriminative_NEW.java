...
//	private int historySize;
	...
	public SemTextFeatureManager_Discriminative_NEW(GlobalNetworkParam param_g, HybridGrammar g, SemTextDataManager dm, FeatureArrayCache cache) {
		super(param_g);
		this._g = g;
		this._dm = dm;
		this._cache = cache;
...
//			return FeatureArray.NEGATIVE_INFINITY;
		...
		if(ids_parent[1]==0 && ids_parent[2]==0 && ids_parent[3]==0 && ids_parent[4]==0){
			...
			int[] fs = new int[]{f};
			FeatureArray fa = FeatureArray.createFeatureArray(fs);
			
			return this._cache.toFeatureArray(fa);
		}
...
//					System.err.println("YES["+phrase+"]");
			...
			for(int historySize = 0; historySize<=NetworkConfig._SEMANTIC_PARSING_NGRAM-1; historySize++)
			{
				...
				fa = this._cache.toFeatureArray(fa);
			}
