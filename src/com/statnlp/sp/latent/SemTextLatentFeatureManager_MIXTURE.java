 ...
 *
 */
public class SemTextLatentFeatureManager_MIXTURE extends FeatureManager{
	...
	public SemTextLatentFeatureManager_MIXTURE(GlobalNetworkParam param_g, HybridGrammar g, SemTextDataManager dm) {
		super(param_g);
		this._g = g;
		this._dm = dm;
		...
		if(ids_parent[1]==0 && ids_parent[2]==0 && ids_parent[3]==0 && ids_parent[4]==0 && ids_parent[5]==0 && ids_parent[6]==0){
			...
			if(pattern_parent.isw()){
				...
				if(_considerXYAsWords){
					String output = p_unit.toString()+"|"+cept;
					int[] fs = new int[1];
					fs[0] = this._param_g.toFeature("unit+cept->word", output, "[X]");
					return FeatureArray.createFeatureArray(fs);
				}
				return FeatureArray.EMPTY;
			}
				...
				if(_considerXYAsWords){
					String output = p_unit.toString()+"|"+cept;
					int[] fs = new int[1];
					fs[0] = this._param_g.toFeature("unit+cept->word", output, "[Y]");
					return FeatureArray.createFeatureArray(fs);
				}
				return FeatureArray.EMPTY;
			}
				...
				if(_considerENDAsWord){
					String output = p_unit.toString()+"|"+cept;
					int[] fs = new int[1];
					fs[0] = this._param_g.toFeature("unit+cept->word", output, "[END]");
					return FeatureArray.createFeatureArray(fs);
				}
				return FeatureArray.EMPTY;
			}
