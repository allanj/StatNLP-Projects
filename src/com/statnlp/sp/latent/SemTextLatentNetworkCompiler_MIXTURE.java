 ...
 *
 */
public class SemTextLatentNetworkCompiler_MIXTURE extends NetworkCompiler{
	...
	public SemTextLatentNetworkCompiler_MIXTURE(HybridGrammar g, SemanticForest forest, SemTextDataManager dm){
					...
					if(eIndex == bIndex+1){
						...
						if(_buildLatent){
							for(int type_id = 0; type_id<2; type_id++){
								long node_middle = this.toNode_middle(bIndex, eIndex, forestNode, p, type_id);
								network.addNode(node_middle);
								network.addEdge(node_above, new long[]{node_middle});
							}
							for(int latent = 0; latent<_numLatent; latent++){
								...
								for(int type_id = 0; type_id<2; type_id++){
									long node_middle = this.toNode_middle(bIndex, eIndex, forestNode, p, type_id);
									network.addNode(node_middle);
									network.addEdge(node_middle, new long[]{node_latent});
								}
								network.addEdge(node_latent, new long[]{node_below});
							}
						} else {
...
//		System.exit(1);
	...
	private long toNode_middle(int bIndex, int eIndex, SemanticForestNode node, HybridPattern p, int type_id){
		return NetworkIDMapper.toHybridNodeID(new int[]{eIndex, eIndex-bIndex, node.getHIndex(), node.getWIndex(), p.getId(), 3, type_id, Network.NODE_TYPE.sum.ordinal()});
	}
