...
//		DPConfig.writeWeight = false;
		...
		Transformer tran = new D2DTransformer();
		...
		D2DInstance[] trainingInsts = null;
		D2DInstance[] testingInsts = null;
		if(dataTypeSet.contains(DPConfig.dataType)){
			trainingInsts = D2DReader.readCNN(trainingPath, true,trainNumber,tran);
			testingInsts = D2DReader.readCNN(decodePath, false,testNumber,tran);
		}else{
			trainingInsts = D2DReader.readInstance(trainingPath, true,trainNumber,selectedEntities,tran, false);
			testingInsts = D2DReader.readInstance(decodePath, false,testNumber,selectedEntities,tran, false);
		}
...
//		System.exit(0);
		...
		FeatureManager dfm = new D2DFeatureManager(new GlobalNetworkParam(),entities);
		D2DNetworkCompiler dnc = new D2DNetworkCompiler(typeMap, viewer);
