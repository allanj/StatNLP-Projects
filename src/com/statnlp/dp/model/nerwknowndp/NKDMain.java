...
//		DPConfig.writeWeight = false;
		...
		Transformer tran = new NKDTransformer();
		...
		NKDInstance[] trainingInsts = null;
		NKDInstance[] testingInsts = null;
		if(dataTypeSet.contains(DPConfig.dataType)){
			trainingInsts = NKDReader.readCNN(trainingPath, true,trainNumber,tran);
			testingInsts = NKDReader.readCNN(decodePath, false,testNumber,tran);
		}else{
			trainingInsts = NKDReader.readInstance(trainingPath, true,trainNumber,selectedEntities,tran, false);
			testingInsts = NKDReader.readInstance(decodePath, false,testNumber,selectedEntities,tran, false);
		}
...
//		System.exit(0);
		...
		FeatureManager dfm = new NKDFeatureManager(new GlobalNetworkParam(),entities);
		NKDNetworkCompiler dnc = new NKDNetworkCompiler(typeMap, viewer);
