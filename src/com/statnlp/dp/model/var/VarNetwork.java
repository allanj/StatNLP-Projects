...
public class VarNetwork extends DependencyNetwork {
	...
	public VarNetwork(int networkId, VarInstance inst, LocalNetworkParam param) {
		super(networkId, inst, param);
		// TODO Auto-generated constructor stub
	}

	public VarNetwork(int networkId, VarInstance inst, long[] nodes, int[][][] children, LocalNetworkParam param, int numNodes) {
	...
	public double totalLossUpTo(int k, int[] child_k){
		VarInstance instance = (VarInstance)this.getInstance();
		...
		if(child_k.length==1 && typeIdx==VarMain.typeMap.get(PARENT_IS+OE) && carr[4]>1 && carr[4]<6 ){
			double err = 0;
			for(int i=left;i<=right;i++){
				String e = sent.get(i).getEntity().length()<2? DPConfig.ONE: sent.get(i).getEntity().substring(2);
				int eIdx = VarMain.typeMap.get(e);
				if(carr[4]!=eIdx)
					err++;
			}
			return err;
