package com.statnlp.dp.model.td;
...
public class TDFeatureManager extends FeatureManager {
	...
	public TDFeatureManager(GlobalNetworkParam param_g, String[] types) {
		super(param_g);
		this.types = types;
	}
			...
			if((pa_type.equals(OE)||pa_type.equals("null")) && !child_1_type.equals(OE) && !child_1_type.equals(ONE)){
				...
				if( (leftIndex==rightIndex && direction==1) || completeness==0){
					for(int i=leftIndex;i<=rightIndex;i++){
						...
						String nextWord = i<sent.length()-1? sent.get(i+1).getName():"END";
						String nextTag = i<sent.length()-1? sent.get(i+1).getTag():"END";
						String child_type = i==leftIndex?E_B_PREFIX+child_1_type:E_I_PREFIX+child_1_type;
