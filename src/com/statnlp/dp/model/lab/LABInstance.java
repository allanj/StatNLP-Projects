...
public class LABInstance extends DependInstance {

	private static final long serialVersionUID = 2294305587493291137L;
	
	public LABInstance(int instanceId, double weight) {
		super(instanceId, weight);
	}
	
	public LABInstance(int instanceId, double weight, Sentence sent) {
		super(instanceId, weight, sent);
	}


	public LABInstance(int instanceId, double weight, Sentence sentence, ArrayList<UnnamedDependency> dependencies, Tree dependencyRoot, Tree output) {
		super(instanceId, weight,sentence,dependencies,dependencyRoot,output);
		
	}
	
	@Override
	public LABInstance duplicate() {
		LABInstance di = new LABInstance(this._instanceId, this._weight,this.sentence);
		if(output==null)
			di.output = null;
...
//		System.err.println("left:"+pa_leftIndex+", right:"+pa_rightIndex);
	...
	public String[] toEntities(Tree spanRoot){
		...
		es[0] = O_TYPE;
		for(int i=1;i<es.length;i++) es[i] = O_TYPE;
