...
//		testFile = trainingPath;
		...
		LNERTransformer trans = new LNERTransformer();
		LNERInstance[] trainingInsts = null;
		LNERInstance[] testingInsts = null;
		if(dataTypeSet.contains(DPConfig.dataType)){
			trainingInsts = LNERReader.readCNN(trainingPath, true, trainNumber, trans);
			testingInsts = LNERReader.readCNN(testFile, false,testNumber,trans);
		}else{
			trainingInsts = LNERReader.readInstance(trainingPath, true,trainNumber,entities,trans);
...
//			System.err.println("Number of Labels:"+DepLabel.LABELS.size());
		...
		LNERFeatureManager dfm = new LNERFeatureManager(new GlobalNetworkParam(), isPipe);
		LNERNetworkCompiler dnc = new LNERNetworkCompiler();
