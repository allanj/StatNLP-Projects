...
public class HBDNetwork extends DependencyNetwork {
	...
	public double totalLossUpTo(int k, int[] child_k){
		HBDInstance instance = (HBDInstance)this.getInstance();
		...
		if(child_k.length==1 && typeIdx==HBDMain.typeMap.get(PARENT_IS+OE) && carr[4]>1 && carr[4]<6 ){
			double err = 0;
			for(int i=left;i<=right;i++){
				String e = sent.get(i).getEntity().length()<2? DPConfig.ONE: sent.get(i).getEntity().substring(2);
				int eIdx = HBDMain.typeMap.get(e);
				if(carr[4]!=eIdx)
					err++;
			}
			return err;
