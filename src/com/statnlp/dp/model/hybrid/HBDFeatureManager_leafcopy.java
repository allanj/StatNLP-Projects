...
public class HBDFeatureManager_leafcopy extends FeatureManager {
	...
	public HBDFeatureManager_leafcopy(GlobalNetworkParam param_g, String[] types) {
		super(param_g);
		this.types = types;
	}
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k) {
		...
		if(children_k.length==1 && !type.equals(OE) && !type.startsWith(PARENT_IS) && !child_1_type.equals(OE) && !child_1_type.startsWith(PARENT_IS))
			return extractLinearFeatures(network,parentArr,childArr_1);
	...
	private FeatureArray extractLinearFeatures(Network network, int[] paArr, int[] childArr){
		ArrayList<Integer> featureList = new ArrayList<Integer>();
		Sentence sent = ((HBDInstance)network.getInstance()).getInput();
		int paPos = paArr[0] - paArr[1];
		String paType = types[paArr[4]];
		String att = paArr[3]==1? "RA":"LA";
		int cPos = childArr[0] - childArr[1];
		String cType = types[childArr[4]];

		addLocalEntityFeatures(network,FEATYPE.entity, featureList, sent, paPos, paType, att);
		if(cPos==paPos){
			if(paType.equals(cType)){
				addLocalEntityFeatures(network,FEATYPE.entity, featureList, sent, paPos, paType);
			}else{
				String word = sent.get(paPos).getName();
				String tag = sent.get(paPos).getTag();
				String prevWord = paPos>1?sent.get(paPos-1).getName():"STR";
				String prevTag = paPos>1?sent.get(paPos-1).getTag():"STR";
				String nextWord = paPos<sent.length()-1? sent.get(paPos+1).getName():"END";
				String nextTag = paPos<sent.length()-1? sent.get(paPos+1).getTag():"END";
				featureList.add(this._param_g.toFeature(network, FEATYPE.entity.name(), "diff-E-W", cType+":"+paType+":"+word));
				featureList.add(this._param_g.toFeature(network, FEATYPE.entity.name(), "diff-E-T", cType+":"+paType+":"+tag));
				featureList.add(this._param_g.toFeature(network, FEATYPE.entity.name(), "diff-E-LW", cType+":"+paType+":"+prevWord));
				featureList.add(this._param_g.toFeature(network, FEATYPE.entity.name(), "diff-E-LT", cType+":"+paType+":"+prevTag));
				featureList.add(this._param_g.toFeature(network, FEATYPE.entity.name(), "diff-E-RW", cType+":"+paType+":"+nextWord));
				featureList.add(this._param_g.toFeature(network, FEATYPE.entity.name(), "diff-E-RT", cType+":"+paType+":"+nextTag));
				featureList.add(this._param_g.toFeature(network, FEATYPE.entity.name(), "diff-E-LT-T", cType+":"+paType+":"+prevTag+","+tag));
			}
		}else{
			addPairwiseFeatures(network,FEATYPE.entity, featureList,sent,paPos,cType,paType);
		}
	...
	private void addLocalEntityFeatures(Network network,FEATYPE fea, ArrayList<Integer> featureList,Sentence sent, int pos, String type){
		int i = pos;
		String word = sent.get(i).getName();
		String tag = sent.get(i).getTag();
		String prevWord = i>1?sent.get(i-1).getName():"STR";
		...
		String nextWord = i<sent.length()-1? sent.get(i+1).getName():"END";
		String nextTag = i<sent.length()-1? sent.get(i+1).getTag():"END";
		String child_type = type;
		featureList.add(this._param_g.toFeature(network, fea.name(), "EW", child_type+":"+word));
		featureList.add(this._param_g.toFeature(network, fea.name(), "ET", child_type+":"+tag));
		featureList.add(this._param_g.toFeature(network, fea.name(), "ELW", child_type+":"+prevWord));
		featureList.add(this._param_g.toFeature(network, fea.name(), "ELT", child_type+":"+prevTag));
		featureList.add(this._param_g.toFeature(network, fea.name(), "ERW", child_type+":"+nextWord));
		featureList.add(this._param_g.toFeature(network, fea.name(), "ERT", child_type+":"+nextTag));
		featureList.add(this._param_g.toFeature(network, fea.name(), "ELT-T",child_type+":"+prevTag+","+tag));
		...
		for(int plen = 1;plen<=6;plen++){
			if(word.length()>=plen){
				String suff = word.substring(word.length()-plen, word.length());
				String pref = word.substring(0,plen);
				featureList.add(this._param_g.toFeature(network, fea.name(), "E-PATTERN-SUFF-"+plen, child_type+":"+suff));
				featureList.add(this._param_g.toFeature(network, fea.name(), "E-PATTERN-PREF-"+plen, child_type+":"+pref));
			}
		}
	}

	
	private void addLocalEntityFeatures(Network network,FEATYPE fea, ArrayList<Integer> featureList,Sentence sent, int pos, String type, String att){
		int i = pos;
		String word = sent.get(i).getName();
		String tag = sent.get(i).getTag();
		String prevWord = i>1?sent.get(i-1).getName():"STR";
		...
		String nextTag = i<sent.length()-1? sent.get(i+1).getTag():"END";
		String child_type = type;
		featureList.add(this._param_g.toFeature(network, fea.name(), "EW-dir", child_type+":"+word+":"+att));
		featureList.add(this._param_g.toFeature(network, fea.name(), "ET-dir", child_type+":"+tag+":"+att));
		featureList.add(this._param_g.toFeature(network, fea.name(), "ELW-dir", child_type+":"+prevWord+":"+att));
		featureList.add(this._param_g.toFeature(network, fea.name(), "ELT-dir", child_type+":"+prevTag+":"+att));
		featureList.add(this._param_g.toFeature(network, fea.name(), "ERW-dir", child_type+":"+nextWord+":"+att));
		featureList.add(this._param_g.toFeature(network, fea.name(), "ERT-dir", child_type+":"+nextTag+":"+att));
		featureList.add(this._param_g.toFeature(network, fea.name(), "ELT-T-dir",child_type+":"+prevTag+","+tag+":"+att));
	...
	private void addPairwiseFeatures(Network network,FEATYPE fea, ArrayList<Integer> featureList,Sentence sent, int pos, String prevType, String currType){
		String prevEntity = prevType;
		String currEn = currType;
		...
		featureList.add(this._param_g.toFeature(network,fea.name(), "E-prev-E",prevEntity+":"+currEn));
		featureList.add(this._param_g.toFeature(network,fea.name(), "currW-prevE-currE",currWord+":"+prevEntity+":"+currEn));
		featureList.add(this._param_g.toFeature(network,fea.name(), "prevW-prevE-currE",lw+":"+prevEntity+":"+currEn));
		featureList.add(this._param_g.toFeature(network,fea.name(), "nextW-prevE-currE",rw+":"+prevEntity+":"+currEn));
		
		featureList.add(this._param_g.toFeature(network,fea.name(), "currT-prevE-currE",currTag+":"+prevEntity+":"+currEn));
		featureList.add(this._param_g.toFeature(network,fea.name(), "prevT-prevE-currE",lt+":"+prevEntity+":"+currEn));
		featureList.add(this._param_g.toFeature(network,fea.name(), "nextT-prevE-currE",rt+":"+prevEntity+":"+currEn));
		featureList.add(this._param_g.toFeature(network,fea.name(), "prevT-currT-prevE-currE",lt+":"+currTag+":"+prevEntity+":"+currEn));
	}
}
