...
/**
 * The hybrid structure of linear chain and div
 * @author zhanming_jie
 *
 */
public class HBDMain {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		String modelType = MODEL.HYBRID.name();
		...
		String jointLinear = DPConfig.data_prefix+modelType+middle+DPConfig.jointlinear_eval_suffix;
...
//		DPConfig.writeWeight = false;
		...
		HBDTransformer tran = new HBDTransformer();
		...
		HBDInstance[] trainingInsts = null;
		HBDInstance[] testingInsts = null;
		if(dataTypeSet.contains(DPConfig.dataType)){
			trainingInsts = HBDReader.readCNN(trainingPath, true,trainNumber,tran);
			testingInsts = HBDReader.readCNN(decodePath, false,testNumber,tran);
		}else{
			trainingInsts = HBDReader.readInstance(trainingPath, true,trainNumber,selectedEntities,tran, false);
			testingInsts = HBDReader.readInstance(decodePath, false,testNumber,selectedEntities,tran, false);
		}
...
//		System.exit(0);
		...
		FeatureManager dfm = new HBDFeatureManager_leafcopy(new GlobalNetworkParam(),entities);
		HBDNetworkCompiler dnc = new HBDNetworkCompiler(typeMap, viewer);
		...
		/****************Evaluation Part**************/
		System.out.println("*****Evaluation Processing*****");
		...
		System.out.println("*****Evaluating Joint Linear Result*******");
		HBDEvaluator.evalJointLinear(predInsts, jointLinear);
	}
