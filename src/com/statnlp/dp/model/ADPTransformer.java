...
public class ADPTransformer extends Transformer{

	

	/**
	 * Construct a span tree from a sentence and the corresponding dependency tree
	 * @param dependencyRoot : Dependency Tree with each word as a node. 
	 * @param sentence
	 * @return
	 */
	@Override
	public Tree toSpanTree(Tree dependencyRoot, Sentence sentence){
		...
		constructSpanTree(spanTreeERoot,dependencyRoot, sentEntities);
		return spanTreeRoot;
	}
	 ...
	 * @param sentence
	 */
	private void constructSpanTree(Tree currentSpan, Tree currentDependency,String[] sentEntities){
		...
		if(pa_leftIndex==pa_rightIndex) {
			Tree leafE = new LabeledScoredTreeNode();
			CoreLabel cl = new CoreLabel();
			cl.setValue(pa_leftIndex+","+pa_rightIndex+","+pa_direction+","+pa_completeness+","+sentEntities[pa_leftIndex]);
			leafE.setLabel(cl);
			currentSpan.addChild(leafE);
			return;
		}
		if(pa_completeness==0){
			if(pa_direction==1){
				...
				leftSubSpanLabel.setValue(pa_leftIndex+","+maxSentIndex+",1,1,"+leftType);
				...
				CoreLabel rightSpanSubLabel = new CoreLabel(); rightSpanSubLabel.setValue((maxSentIndex+1)+","+pa_rightIndex+",0,1,"+rightType);
				...
				constructSpanTree(leftChildSubSpan, currentDependency,sentEntities);
				constructSpanTree(rightChildSubSpan, copyLastChildWord,sentEntities);
			}else{
				...
				CoreLabel leftSpanSubLabel = new CoreLabel(); leftSpanSubLabel.setValue(pa_leftIndex+","+(minIndex-1)+",1,1,"+leftType);
				...
				CoreLabel rightSpanSubLabel = new CoreLabel(); rightSpanSubLabel.setValue(minIndex+","+pa_rightIndex+",0,1,"+rightType);
				rightChildSubSpan.setLabel(rightSpanSubLabel);
				rightChildSpan.addChild(rightChildSubSpan);
				...
				currentSpan.addChild(leftChildSpan);
				currentSpan.addChild(rightChildSpan);
				constructSpanTree(leftChildSubSpan, copyFirstChildWord,sentEntities);
				constructSpanTree(rightChildSubSpan, currentDependency,sentEntities);
			}
		}else{
			if(pa_direction==1){
				Tree lastChildWord = currentDependency.lastChild();
				...
				boolean isMixed = false;
				String leftType = sentEntities[pa_leftIndex];
				for(int i=pa_leftIndex+1;i<=lastChildWordIndex-1;i++)
					if(!leftType.equals(sentEntities[i])) {isMixed = true; break;}
				leftType = isMixed? "O":leftType;
				CoreLabel leftSpanLabel = new CoreLabel(); leftSpanLabel.setValue(pa_leftIndex+","+lastChildWordIndex+",1,0,EMPTY");
				leftChildSpan.setLabel(leftSpanLabel);
				
				CoreLabel leftSpanSubLabel = new CoreLabel(); leftSpanSubLabel.setValue(pa_leftIndex+","+lastChildWordIndex+",1,0,"+leftType);
				leftChildSubSpan.setLabel(leftSpanSubLabel);
				leftChildSpan.addChild(leftChildSubSpan);
				...
				CoreLabel rightSpanSubLabel = new CoreLabel(); rightSpanSubLabel.setValue(lastChildWordIndex+","+pa_rightIndex+",1,1,"+rightType);
				rightChildSubSpan.setLabel(rightSpanSubLabel);
				rightChildSpan.addChild(rightChildSubSpan);
				...
				boolean isMixed = false;
				String leftType = sentEntities[pa_leftIndex];
				for(int i=pa_leftIndex+1;i<=firstChildWordIndex-1;i++)
					if(!leftType.equals(sentEntities[i])) {isMixed = true; break;}
				leftType = isMixed? "O":leftType;
				CoreLabel leftSpanLabel = new CoreLabel(); leftSpanLabel.setValue(pa_leftIndex+","+firstChildWordIndex+",0,1,EMPTY");
				leftChildSpan.setLabel(leftSpanLabel);
				
				CoreLabel leftSpanSubLabel = new CoreLabel(); leftSpanSubLabel.setValue(pa_leftIndex+","+firstChildWordIndex+",0,1,"+leftType);
				leftChildSubSpan.setLabel(leftSpanSubLabel);
				leftChildSpan.addChild(leftChildSubSpan);
				...
				CoreLabel rightSpanSubLabel = new CoreLabel(); rightSpanSubLabel.setValue(firstChildWordIndex+","+pa_rightIndex+",0,0,"+rightType);
				rightChildSubSpan.setLabel(rightSpanSubLabel);
				rightChildSpan.addChild(rightChildSubSpan);
