...
public class BitextNetworkConstructor {
	...
	public int countNodes(int srcLen, int tgtLen){
		long node = this.toNetworkID(0, srcLen, 0, tgtLen, TYPE.N00.ordinal());
//		System.err.println(NetworkIDMapper.viewHybridNode_bitext(node));
...
//		return _cache_children[this._maxSrcLength-1][this._maxTgtLength-1];
	...
	public static void main(String args[]){
		...
		while(curr!=null){
			int srcLen = curr[0];
			int tgtLen = curr[1];
			BitextNetworkConstructor c = new BitextNetworkConstructor(srcLen, tgtLen);
	...
	public BitextNetworkConstructor(int maxSrcLength, int maxTgtLength){
		this._maxSrcLength = maxSrcLength;
		this._maxTgtLength = maxTgtLength;
		this.init();
	}

	public BitextNetworkConstructor(int maxSrcLength, int maxTgtLength, double[][] membership){
		this._maxSrcLength = maxSrcLength;
		this._maxTgtLength = maxTgtLength;
...
//		System.err.println(srcLen+","+srcBIndex+","+tgtLen+","+tgtBIndex+","+type);
	...
	public void build_topdown_exact(){
		this.build(0, this._maxSrcLength, 0, this._maxTgtLength, new int[]{this._maxSrcLength, this._maxTgtLength}, TYPE.N00);
		if(this._numNodes<=0){
	...
	public void build_topdown_BITG(int BITG_MAX_LEN){
		this.BITG_MAX_LEN = BITG_MAX_LEN;
		int k = 0;
		int[] curr = new int[]{1,1};
		while(curr!=null){
			this.build_BITG(0, curr[0], 0, curr[1], curr, TYPE.N00);
	...
	public void build_topdown(){
		...
		int[] curr = new int[]{1,1};
		while(curr!=null){
			this.build(0, curr[0], 0, curr[1], curr, TYPE.N00);
			System.err.println(curr[0]+","+curr[1]+"\t"+this._numNodes+","+this._numEdges+"\t"+(curr[0]+curr[1]));
			this._avgNumNodes += this._numNodes;
			this._avgNumEdges += this._numEdges;
			curr = nextPair(curr, this._maxSrcLength, this._maxTgtLength);
	...
	private int BITG_MAX_LEN = 6;

	public int build_BITG(int srcBIndex, int srcEIndex, int tgtBIndex, int tgtEIndex, int[] maxIndices, TYPE type){
		...
		if(srcBIndex==maxIndices[0]-1 && srcEIndex==maxIndices[0]){
			if(!(tgtBIndex==maxIndices[1]-1 && tgtEIndex==maxIndices[1])){
				result = -1;
		...
		else if(tgtBIndex==maxIndices[1]-1 && tgtEIndex==maxIndices[1]){
			if(!(srcBIndex==maxIndices[0]-1 && srcEIndex==maxIndices[0])){
				result = -1;
				this.setNodeId(node, -2);
				return result;
		...
		if(srcBIndex==0 && srcEIndex==1){
			if(!(tgtBIndex==0 && tgtEIndex==1)){
				result = -1;
				this.setNodeId(node, -2);
				return result;
			}
		...
		else if(tgtBIndex==0 && tgtEIndex==1){
			if(!(srcBIndex==0 && srcEIndex==1)){
				result = -1;
				this.setNodeId(node, -2);
				return result;
		...
		if(srcLen == 1 && tgtLen <= BITG_MAX_LEN && tgtLen >= 0 && type == TYPE.I00){
			if(_membership!=null){
...
//				result = this.addNode(node);
			...
			if(type == TYPE.N00){
				for(int srcCIndex = srcBIndex+1; srcCIndex < srcEIndex; srcCIndex++){
					for(int tgtCIndex = tgtBIndex+1; tgtCIndex < tgtEIndex; tgtCIndex++){
						int child1 = this.build_BITG(srcBIndex, srcCIndex, tgtBIndex, tgtCIndex, maxIndices, TYPE.I11);
						int child2 = this.build_BITG(srcCIndex, srcEIndex, tgtCIndex, tgtEIndex, maxIndices, TYPE.NBAR);
			...
			if(type == TYPE.I00){
				for(int srcCIndex = srcBIndex+1; srcCIndex < srcEIndex; srcCIndex++){
					for(int tgtCIndex = tgtBIndex+1; tgtCIndex < tgtEIndex; tgtCIndex++){
						int child1 = this.build_BITG(srcBIndex, srcCIndex, tgtCIndex, tgtEIndex, maxIndices, TYPE.N11);
						int child2 = this.build_BITG(srcCIndex, srcEIndex, tgtBIndex, tgtCIndex, maxIndices, TYPE.IBAR);
						if(child1!=-1 && child2!=-1){
							hasChild = true;
							childrenList.add(new int[]{child1, child2});
						}
			...
			if(type == TYPE.NBAR){
				...
				int child = this.build_BITG(srcBIndex, srcEIndex, tgtBIndex, tgtEIndex, maxIndices, TYPE.I00);
				if(child!=-1){
					hasChild = true;
			...
			if(type == TYPE.IBAR){
				...
				int child = this.build_BITG(srcBIndex, srcEIndex, tgtBIndex, tgtEIndex, maxIndices, TYPE.N00);
				if(child!=-1){
					hasChild = true;
					childrenList.add(new int[]{child});
				}
			...
			if(type == TYPE.N10){
				if(tgtBIndex+1<tgtEIndex){
					int child = this.build_BITG(srcBIndex, srcEIndex, tgtBIndex+1, tgtEIndex, maxIndices, TYPE.N10);
					if(child!=-1){
						hasChild = true;
						childrenList.add(new int[]{child});
					}
			...
			if(type == TYPE.N11){
				if(srcBIndex<srcEIndex-1){
					int child = this.build_BITG(srcBIndex, srcEIndex-1, tgtBIndex, tgtEIndex, maxIndices, TYPE.N11);
					if(child!=-1){
						hasChild = true;
						childrenList.add(new int[]{child});
					}
				}
				int child = this.build_BITG(srcBIndex, srcEIndex, tgtBIndex, tgtEIndex, maxIndices, TYPE.N10);
				if(child!=-1){
					hasChild = true;
					childrenList.add(new int[]{child});
				}
			...
			if(type == TYPE.I10){
				if(tgtBIndex<tgtEIndex-1){
					int child = this.build_BITG(srcBIndex, srcEIndex, tgtBIndex, tgtEIndex-1, maxIndices, TYPE.I10);
					if(child!=-1){
						hasChild = true;
						childrenList.add(new int[]{child});
					}
			...
			if(type == TYPE.I11){
				if(srcBIndex<srcEIndex-1){
					int child = this.build_BITG(srcBIndex, srcEIndex-1, tgtBIndex, tgtEIndex, maxIndices, TYPE.I11);
					if(child!=-1){
						hasChild = true;
						childrenList.add(new int[]{child});
					}
				}
				int child = this.build_BITG(srcBIndex, srcEIndex, tgtBIndex, tgtEIndex, maxIndices, TYPE.I10);
				if(child!=-1){
					hasChild = true;
					childrenList.add(new int[]{child});
				}
...
//	
	public int build(int srcBIndex, int srcEIndex, int tgtBIndex, int tgtEIndex, int[] maxIndices, TYPE type){
...
//				result = this.addNode(node);
			...
			if(type == TYPE.N00){
				for(int srcCIndex = srcBIndex+1; srcCIndex < srcEIndex; srcCIndex++){
					for(int tgtCIndex = tgtBIndex+1; tgtCIndex < tgtEIndex; tgtCIndex++){
						int child1 = this.build(srcBIndex, srcCIndex, tgtBIndex, tgtCIndex, maxIndices, TYPE.I11);
						int child2 = this.build(srcCIndex, srcEIndex, tgtCIndex, tgtEIndex, maxIndices, TYPE.NBAR);
						if(child1!=-1 && child2!=-1){
							hasChild = true;
							childrenList.add(new int[]{child1, child2});
						}
			...
			if(type == TYPE.I00){
				for(int srcCIndex = srcBIndex+1; srcCIndex < srcEIndex; srcCIndex++){
					for(int tgtCIndex = tgtBIndex+1; tgtCIndex < tgtEIndex; tgtCIndex++){
						int child1 = this.build(srcBIndex, srcCIndex, tgtCIndex, tgtEIndex, maxIndices, TYPE.N11);
						int child2 = this.build(srcCIndex, srcEIndex, tgtBIndex, tgtCIndex, maxIndices, TYPE.IBAR);
						if(child1!=-1 && child2!=-1){
							hasChild = true;
							childrenList.add(new int[]{child1, child2});
						}
			...
			if(type == TYPE.NBAR){
				...
				int child = this.build(srcBIndex, srcEIndex, tgtBIndex, tgtEIndex, maxIndices, TYPE.I00);
				if(child!=-1){
					hasChild = true;
					childrenList.add(new int[]{child});
				}
			...
			if(type == TYPE.IBAR){
				...
				int child = this.build(srcBIndex, srcEIndex, tgtBIndex, tgtEIndex, maxIndices, TYPE.N00);
				if(child!=-1){
					hasChild = true;
					childrenList.add(new int[]{child});
				}
			...
			if(type == TYPE.N10){
				if(tgtBIndex+1<tgtEIndex){
					int child = this.build(srcBIndex, srcEIndex, tgtBIndex+1, tgtEIndex, maxIndices, TYPE.N10);
					if(child!=-1){
						hasChild = true;
						childrenList.add(new int[]{child});
					}
			...
			if(type == TYPE.N11){
				if(srcBIndex<srcEIndex-1){
					int child = this.build(srcBIndex, srcEIndex-1, tgtBIndex, tgtEIndex, maxIndices, TYPE.N11);
					if(child!=-1){
						hasChild = true;
						childrenList.add(new int[]{child});
					}
				}
				int child = this.build(srcBIndex, srcEIndex, tgtBIndex, tgtEIndex, maxIndices, TYPE.N10);
				if(child!=-1){
					hasChild = true;
					childrenList.add(new int[]{child});
				}
			...
			if(type == TYPE.I10){
				if(tgtBIndex<tgtEIndex-1){
					int child = this.build(srcBIndex, srcEIndex, tgtBIndex, tgtEIndex-1, maxIndices, TYPE.I10);
					if(child!=-1){
						hasChild = true;
						childrenList.add(new int[]{child});
					}
			...
			if(type == TYPE.I11){
				if(srcBIndex<srcEIndex-1){
					int child = this.build(srcBIndex, srcEIndex-1, tgtBIndex, tgtEIndex, maxIndices, TYPE.I11);
					if(child!=-1){
						hasChild = true;
						childrenList.add(new int[]{child});
					}
				}
				int child = this.build(srcBIndex, srcEIndex, tgtBIndex, tgtEIndex, maxIndices, TYPE.I10);
				if(child!=-1){
					hasChild = true;
					childrenList.add(new int[]{child});
				}
...
//		if(srcBIndex==0 && srcEIndex==1 && tgtBIndex==0 && tgtEIndex==1){
