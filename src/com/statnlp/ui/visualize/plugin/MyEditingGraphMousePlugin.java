 ...
 *
 */
public class MyEditingGraphMousePlugin<VNode,VLink> extends EditingGraphMousePlugin<VNode,VLink> {
    

    protected EdgeChecker<VNode,VLink> edgeChecker;
    protected VertexChecker<VNode,VLink> vertexChecker;
    
    
    
    public MyEditingGraphMousePlugin(Factory<VNode> vertexFactory, Factory<VLink> edgeFactory) {
     ...
     * @param modifiers
     */
    public MyEditingGraphMousePlugin(int modifiers, Factory<VNode> vertexFactory, Factory<VLink> edgeFactory) {
	...
	public void mousePressed(MouseEvent e) {
        if(checkModifiers(e)) {
            ...
            if(pickSupport != null) {
            	Graph<VNode,VLink> graph = vv.getModel().getGraphLayout().getGraph();
                ...
                final VNode vertex = pickSupport.getVertex(vv.getModel().getGraphLayout(), p.getX(), p.getY());
                if(vertex != null && this.edgeChecker.edgeMode()) { // get ready to make an edge
                    startVertex = vertex;
                    down = e.getPoint();
                    ...
                    VNode newVertex = vertexFactory.create();
                    Layout<VNode,VLink> layout = vv.getModel().getGraphLayout();
                    boolean okToAdd = true;
                    if (vertexChecker != null) {
                        okToAdd = vertexChecker.vertexMode() && vertexChecker.checkVertex(graph, vv, newVertex);
                    }
                    if (okToAdd) {
	...
	public void mouseReleased(MouseEvent e) {
        if(checkModifiers(e)) {
            ...
            GraphElementAccessor<VNode,VLink> pickSupport = vv.getPickSupport();
            if(pickSupport != null) {
                final VNode vertex = pickSupport.getVertex(layout, p.getX(), p.getY());
                if (vertex != null && startVertex != null) {
                    Graph<VNode, VLink> graph =
                            (Graph<VNode, VLink>) vv.getGraphLayout().getGraph();
                    boolean okToAdd = true;
                    
                    edgeChecker.setEnddingNodes(startVertex, vertex);
          
                    VLink edge = edgeFactory.create();
                    
                    if (edgeChecker != null && edgeChecker.edgeMode()) {
    ...
    public void mouseDragged(MouseEvent e) {
        if(checkModifiers(e)) {
            ...
            VisualizationViewer<VNode,VLink> vv =
                (VisualizationViewer<VNode,VLink>)e.getSource();
            vv.repaint();
        }
    }
    ...
    public void setEdgeChecker(EdgeChecker<VNode, VLink> edgeChecker) {
        this.edgeChecker = edgeChecker;
    }

    public void setVertexChecker(VertexChecker<VNode, VLink> vertexChecker) {
     ...
     * @author Dr. Greg Bernstein
     */
    public interface VertexChecker<VNode, VLink> {
    	
    	boolean vertexMode();

        boolean checkVertex(Graph<VNode, VLink> g, VisualizationViewer<VNode, VLink> vv, VNode vertex);
    }
     ...
     */
    public interface EdgeChecker<VNode, VLink> {
    	
    	boolean setEnddingNodes(VNode start, VNode end);
    	
    	boolean edgeMode();

        boolean checkEdge(Graph<VNode, VLink> g, VisualizationViewer<VNode, VLink> vv, VLink edge, VNode start, VNode end, EdgeType dir);
    }
}
