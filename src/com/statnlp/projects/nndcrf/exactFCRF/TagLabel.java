...
public class TagLabel implements Serializable{
	...
	public static final Map<String, TagLabel> TAGS = new HashMap<String, TagLabel>();
	public static final Map<Integer, TagLabel> TAGS_INDEX = new HashMap<Integer, TagLabel>();
	
	public static TagLabel get(String form){
		if(!TAGS.containsKey(form)){
			if(locked) 
				throw new RuntimeException("Unknown tag type:"+form);
			TagLabel label = new TagLabel(form, TAGS.size());
			TAGS.put(form, label);
	...
	public static TagLabel get(int id){
	...
	public TagLabel(TagLabel lbl){
		this._form = lbl._form;
		this._id = lbl._id;
	}
	
	private TagLabel(String form, int id){
		this._form = form;
		this._id = id;
	}
