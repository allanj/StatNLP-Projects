...
public class MHPFeatureManager extends FeatureManager {
	...
	public MHPFeatureManager(GlobalNetworkParam param_g, String[] types) {
		super(param_g);
		this.types = types;
	}
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k) {
		...
		if(children_k.length==1) throw new RuntimeException("parent is root span, but have "+children_k.length+" children?");

		
		long[] children = new long[2];
		int[][] childrenArr = new int[children.length][];
		String[] childrenLMT = new String[children.length];
		String[] childrenRMT = new String[children.length];
		for(int i=0;i<children.length;i++) {
			children[i] = network.getNode(children_k[i]);
			childrenArr[i] = NetworkIDMapper.toHybridNodeArray(children[i]);
			childrenLMT[i] = types[childrenArr[i][5]];
			childrenRMT[i] = types[childrenArr[i][6]];
		}
		...
		for(int c=0;c<children.length;c++){
			...
			if(cleft==cright && !sent.get(cleft).getName().equals("ROOT") && catt.equals("RA")){
				int i = cleft;
				...
				String child_type = childrenLMT[c];
