...
import com.statnlp.ie.flatsemi.zeroth.FlatSemiZerothFeatureManager;
import com.statnlp.ie.flatsemi.zeroth.FlatSemiZerothInstance;
import com.statnlp.ie.flatsemi.zeroth.FlatSemiZerothModel;
...
public class MentionExtractionExpr_new_ACE2005_semi_NO_TUNING_LIMIT_6_zeroth_EXP {
	...
	public static void main(String args[])throws IOException, ClassNotFoundException{
		...
		FlatSemiZerothConfig._MAX_LBFGS_ITRS = num_itrs;
		FlatSemiZerothConfig._MAX_MENTION_LENGTH = maxLen;
		
		System.err.println("OK12Aug2348");
		
		long bTime_overall = System.currentTimeMillis();
		
		for(String folder : new String[]{"ACE2005"}){
...
//			String subfolder = args[0];
			{
				for(String c : new String[]{"0.01"}){
					...
					FlatSemiZerothFeatureManager fm = new FlatSemiZerothFeatureManager(param);
					
					System.err.println(Arrays.toString(manager.getMentionTemplate().getAllTypes()));
					
					File f = new File("experiments/mention/new_model/"+FlatSemiZerothConfig._type.name()+"/"+folder+"/"+subfolder+"/");
					f.mkdirs();
					
					String filename_output = "experiments/mention/new_model/"+FlatSemiZerothConfig._type.name()+"/"+folder+"/"+subfolder+"/output.data";
					PrintWriter p = new PrintWriter(new File(filename_output));
					
					String filename_input;
					Scanner scan;
					
					filename_input = "data/"+folder+"/data/"+subfolder+"/mention-standard/"+FlatSemiZerothConfig._type.name()+"/train.data";
					scan = new Scanner(new File(filename_input));
					ArrayList<LabeledTextSpan> spans_train = readTextSpans(scan, manager, true);
					
					filename_input = "data/"+folder+"/data/"+subfolder+"/mention-standard/"+FlatSemiZerothConfig._type.name()+"/dev.data";
					scan = new Scanner(new File(filename_input));
					ArrayList<LabeledTextSpan> spans_dev = readTextSpans(scan, manager, false);
					
					filename_input = "data/"+folder+"/data/"+subfolder+"/mention-standard/"+FlatSemiZerothConfig._type.name()+"/test.data";
					scan = new Scanner(new File(filename_input));
					ArrayList<LabeledTextSpan> spans_test = readTextSpans(scan, manager, false);
					
					int train_size = spans_train.size();
					
					ArrayList<FlatSemiZerothInstance> instances = new ArrayList<FlatSemiZerothInstance>();
					for(int k = 0; k<train_size; k++){
						MentionFlatSemiZerothInstance inst = new MentionFlatSemiZerothInstance(k+1, spans_train.get(k), manager.getMentionTemplate());
						instances.add(inst);
					}
					...
					FlatSemiZerothModel model = new FlatSemiZerothModel(fm, manager.getMentionTemplate().getAllTypesExcludingStartAndFinish_arr(), maxLen);
...
//					System.err.println("read.");
					...
					for(int k = 0; k<train_size; k++){
						MentionFlatSemiZerothInstance inst = new MentionFlatSemiZerothInstance(k+1, spans_train.get(k), manager.getMentionTemplate());
						MentionFlatSemiZerothInstance inst_unlabeled = inst.removeOutput();
...
//						System.err.println("lambda="+lambda);
						...
						for(int k = 0; k<spans_test.size(); k++){
							MentionFlatSemiZerothInstance inst = new MentionFlatSemiZerothInstance(k+1, spans_test.get(k), manager.getMentionTemplate());
							MentionFlatSemiZerothInstance inst_unlabeled = inst.removeOutput();
...
//						System.err.println("mention penalty="+fm.getParam().getWeight(f1));
						...
						for(int k = 0; k<spans_dev.size(); k++){
							MentionFlatSemiZerothInstance inst = new MentionFlatSemiZerothInstance(k+1, spans_dev.get(k), manager.getMentionTemplate());
							MentionFlatSemiZerothInstance inst_unlabeled = inst.removeOutput();
