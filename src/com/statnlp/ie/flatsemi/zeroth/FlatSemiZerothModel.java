...
public class FlatSemiZerothModel {
	
	private transient ArrayList<FlatSemiZerothInstance> _labeledInstances;
	private transient FlatSemiZerothBuilder _builder;
	private FlatSemiZerothFeatureManager _fm;
	private SemanticTag[] _tags;
	private boolean _cacheFeatures = true;
	
	public FlatSemiZerothModel(FlatSemiZerothFeatureManager fm, SemanticTag[] tags, int maxLen){
		this._fm = fm;
		...
		this._builder = new FlatSemiZerothBuilder(fm, tags, maxLen);
	}
	
	public void train(ArrayList<FlatSemiZerothInstance> labeledInstances, int max_num_iterations){
...
//		System.err.println("word penalty="+weight2);
	...
	private FlatSemiZerothNetwork[] _networks_labeled;
	private FlatSemiZerothNetwork[] _networks_unlabeled;
	
	private void touch(){
		int num_instances = this._labeledInstances.size();
		FlatSemiZerothNetwork network1;
		FlatSemiZerothNetwork network2;
		for(int k = 0; k<num_instances; k++){
			if(k%1000==0 && k!=0){
				System.err.println(k+"...");
			}
			FlatSemiZerothInstance inst_labeled = this._labeledInstances.get(k);
...
//			System.err.println(inst_labeled.getId());
			...
			FlatSemiZerothInstance inst_unlabeled = inst_labeled.removeOutput();
	...
	private FlatSemiZerothNetwork getNetwork(FlatSemiZerothInstance inst){
		
		if(this._cacheNetworks && this._networks_labeled == null){
//			System.err.println("xxx"+_labeledInstances.size());
			this._networks_labeled = new FlatSemiZerothNetwork[this._labeledInstances.size()];
			this._networks_unlabeled = new FlatSemiZerothNetwork[this._labeledInstances.size()];
		}
		...
		FlatSemiZerothNetwork network = this._builder.build(inst);
...
//		this._fm.getParam().preview();
	...
	public void max(FlatSemiZerothInstance inst){
...
//		IELinearNetwork network = this._builder.build(inst);
		FlatSemiZerothNetwork network = this.getNetwork(inst);
		network.max();
...
//		System.err.println(span.viewPredict());
	...
	private void decode(FlatSemiZerothNetwork network, int node_k, UnlabeledTextSpan span){
		
		long node = network.get(node_k);
		int[] array = NetworkIDMapper.toHybridNodeArray(node);
		int hybridType = array[4];
		if(hybridType == FlatSemiZerothConfig.NODE_TYPE.EXACT.ordinal())
		{
			int eIndex = array[0];
