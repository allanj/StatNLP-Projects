...
public class Evaluator_CRFPP_BLIOU_CC_Head_Alternative {
...
//		IEManager manager = EventExtractionReader.readIEManager(filename_template);
		...
		for(String folder: new String[]{"ACE2004", "ACE2005"}){
			for(String subfolder : new String[]{"English"}){
				for(String overlap : new String[]{"cascaded"}){
...
//							System.err.println("k="+k);
							...
							String test_pred = "experiments/mention/lcrf-bliou/"+IEConfig._type.name()+"/"+folder+"/"+subfolder+"/"+overlap+"/output-dev-"+k+".data";
					...
					}
					{
						...
						for(int k = 1; k<=7; k++){
							String test_pred = "experiments/mention/lcrf-bliou/"+IEConfig._type.name()+"/"+folder+"/"+subfolder+"/"+overlap+"/output-test-"+k+".data";
							scan = new Scanner(new File(test_pred));
...
//		toSpan_FIRST(v, words, preds, span, manager);
		toSpan_LAST(v, words, preds, span, manager);
//		toSpan_FULL(v, words, preds, span, manager);
		
	}


	private static void toSpan_LAST(double[] v, ArrayList<AttributedWord> words, ArrayList<String> preds, UnlabeledTextSpan span, IEManager manager){
				...
				for(String tag : tags){
					if(tag.startsWith("U-")){
						...
						for(int j = i+1; j<=preds.size(); j++){
							if(j==preds.size()){
								span.label_predict(i, j, new Mention(i,j,j-1, j, manager.toMentionType(type)));
							} else {
								String tag_new = preds.get(j);
								if(tag_new.indexOf("L-"+type)>=0){
									boolean f = span.label_predict(i, j+1, new Mention(i,j+1, j, j+1, manager.toMentionType(type)));
//									if(!f){
...
//									}
	...
	private static void toSpan_FIRST(double[] v, ArrayList<AttributedWord> words, ArrayList<String> preds, UnlabeledTextSpan span, IEManager manager){
			...
			if(pred.equals("O")){
				...
				for(String tag : tags){
					if(tag.startsWith("U-")){
						...
						for(int j = i+1; j<=preds.size(); j++){
							if(j==preds.size()){
								span.label_predict(i, j, new Mention(i,j,i, i+1, manager.toMentionType(type)));
							} else {
								String tag_new = preds.get(j);
								if(tag_new.indexOf("L-"+type)>=0){
									boolean f = span.label_predict(i, j+1, new Mention(i,j+1,i, i+1, manager.toMentionType(type)));
//									if(!f){
//										throw new RuntimeException("x"+i+","+j+"="+type+"\t"+span.toLine());
//									}
									break;
	...
	private static void toSpan_FULL(double[] v, ArrayList<AttributedWord> words, ArrayList<String> preds, UnlabeledTextSpan span, IEManager manager){
					...
					if(tag.startsWith("U-")){
						...
						for(int j = i+1; j<=preds.size(); j++){
							if(j==preds.size()){
								...
								String tag_new = preds.get(j);
								if(tag_new.indexOf("L-"+type)>=0){
									boolean f = span.label_predict(i, j+1, new Mention(i,j+1,i, j+1, manager.toMentionType(type)));
//									if(!f){
//										throw new RuntimeException("x"+i+","+j+"="+type+"\t"+span.toLine());
//									}
									break;
