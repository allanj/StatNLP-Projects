...
public class IELinearModel {
	
	private transient ArrayList<IELinearInstance> _labeledInstances;
	private transient IELinearBuilder _builder;
	private IELinearFeatureManager _fm;
	private SemanticTag[] _tags;
	private boolean _cacheFeatures = true;
	
	public IELinearModel(IELinearFeatureManager fm, SemanticTag[] tags){
		this._fm = fm;
		this._tags = tags;
		NetworkConfig.TRAIN_MODE_IS_GENERATIVE = false;
		this._builder = new IELinearBuilder(fm, tags);
	}
	
	public void train(ArrayList<IELinearInstance> labeledInstances){
	...
	private IELinearNetwork[] _networks_labeled;
	private IELinearNetwork[] _networks_unlabeled;
	
	private void touch(){
		int num_instances = this._labeledInstances.size();
		IELinearNetwork network1;
		IELinearNetwork network2;
...
//			System.err.println(inst_labeled.getId());
			...
			IELinearInstance inst_unlabeled = inst_labeled.removeOutput();
...
//			}
	...
	private void train(){
		for(int it = 0; it<IELinearConfig._MAX_LBFGS_ITRS; it++){
	...
	private IELinearNetwork getNetwork(IELinearInstance inst){
		
		if(this._cacheNetworks && this._networks_labeled == null){
//			System.err.println("xxx"+_labeledInstances.size());
			this._networks_labeled = new IELinearNetwork[this._labeledInstances.size()];
			this._networks_unlabeled = new IELinearNetwork[this._labeledInstances.size()];
		}
...
//		IELinearNetwork network = this._builder.build(inst);
		IELinearNetwork network = this.getNetwork(inst);
		network.max();
