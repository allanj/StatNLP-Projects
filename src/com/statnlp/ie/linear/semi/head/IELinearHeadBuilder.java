...
public class IELinearHeadBuilder {
	...
	public IELinearHeadBuilder(IELinearFeatureManager fm, SemanticTag[] tags){
		this._fm = fm;
		this._tags = tags;
...
//		System.err.println("size="+nodes_exact_start_tags.length);
	...
	private long toNode_incomplete_start_tag_before_head(int bIndex, SemanticTag tag){
		int srcHeight = - bIndex;
		int srcWidth = 0;
		int tgtHeight = srcHeight;
		int tgtWidth = tag.getId() + 1;
		int hybridType = IELinearConfig.NODE_TYPE.INCOMPLETE_START_TAG_BEFORE_HEAD.ordinal();
		return NetworkIDMapper.toHybridNodeID(new int[]{srcHeight, srcWidth, tgtHeight, tgtWidth, hybridType});
	}
	
	private long toNode_incomplete_start_tag_within_head(int bIndex, SemanticTag tag){
		int srcHeight = - bIndex;
		int srcWidth = 0;
		int tgtHeight = srcHeight;
		int tgtWidth = tag.getId() + 1;
		int hybridType = IELinearConfig.NODE_TYPE.INCOMPLETE_START_TAG_WITHIN_HEAD.ordinal();
		return NetworkIDMapper.toHybridNodeID(new int[]{srcHeight, srcWidth, tgtHeight, tgtWidth, hybridType});
	}
	
	private long toNode_incomplete_start_tag_after_head(int bIndex, SemanticTag tag){
		int srcHeight = - bIndex;
		int srcWidth = 0;
		int tgtHeight = srcHeight;
		int tgtWidth = tag.getId() + 1;
		int hybridType = IELinearConfig.NODE_TYPE.INCOMPLETE_START_TAG_AFTER_HEAD.ordinal();
		return NetworkIDMapper.toHybridNodeID(new int[]{srcHeight, srcWidth, tgtHeight, tgtWidth, hybridType});
	}
