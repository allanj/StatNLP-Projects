...
public class IELinearLatentNetworkCompiler extends NetworkCompiler{
	...
	private HashMap<SemanticTag, Integer> _tag2numLatent;
	
	private int _K = 2;
	
	public IELinearLatentNetworkCompiler(SemanticTag[] tags){
		System.err.println("K="+this._K);
		this._tags = tags;
		this._N = IELinearLatentConfig._MAX_SENT_LENGTH;
		this._M = this._tags.length;
		
		this._tag2numLatent = new HashMap<SemanticTag, Integer>();
		for(int k = 0; k<tags.length; k++){
			this._tag2numLatent.put(tags[k], this._K);
		}
	}
	
	@Override
	public IELinearLatentNetwork compile(int networkId, Instance inst, LocalNetworkParam param) {
		
		if(inst.isLabeled()){
//			System.err.println("Ok..labeled..");
			IELinearLatentNetwork network = this.build_labeled(networkId, (IELinearLatentInstance)inst, param);
...
//			System.err.println("Ok..unlabeled");
			...
			int numNodes = this.getRootPositionForLen(inst.size())+1;
			IELinearLatentNetwork network = new IELinearLatentNetwork(networkId, (IELinearLatentInstance)inst, this._nodes, this._children, param, numNodes);
//			System.err.println("unlabeled.#nodes="+numNodes);
	...
	public IELinearLatentNetwork build_labeled(int id, IELinearLatentInstance inst, LocalNetworkParam param){
		
		IELinearLatentNetwork network = new IELinearLatentNetwork(id, inst, param);
...
//	}
	...
	public void build_unlabeled(){
		IELinearLatentNetwork network = new IELinearLatentNetwork();
		this.build(network, -1);
		network.finalizeNetwork();
		this._nodes = network.getAllNodes();
		this._children = network.getAllChildren();
		int size = 1 + (this._M * (1 + this._K * 2) + 3) * this._N;
		...
		for(int k = 1; k<=this._N; k++){
			this._roots[k-1] = 1 + (this._M * (1 + this._K * 2) + 3) * k - 1;
//			System.err.println(this._roots[k-1]+":"+NetworkIDMapper.viewHybridNode_ie(this._nodes[this._roots[k-1]], this._N, this._M));
//			System.err.println(this._roots[k-1]+":"+Arrays.toString(NetworkIDMapper.toHybridNodeArray(this._nodes[this._roots[k-1]])));
		}
//		System.exit(1);
...
//		System.err.println(":"+NetworkIDMapper.viewHybridNode_ie(this._nodes[this._nodes.length-1], this._N, this._M));
	...
	public void build_labeled(IELinearLatentNetwork network, IELinearLatentInstance inst){
...
//			System.exit(1);
			for(int k = 0; k<this._tags.length; k++){
				...
				int numLatent = this._tag2numLatent.get(tag);
				
				for(int latent = 0; latent<numLatent; latent++){
					long node_exact_start_tag_latent = this.toNode_exact_start_tag_latent(bIndex, tag, latent);
					network.addNode(node_exact_start_tag_latent);
					network.addEdge(node_exact_start_tag, new long[]{node_exact_start_tag_latent});
				}
				...
				if(this._isLabeled){
					...
					for(int latent = 0; latent<numLatent; latent++){
						long node_exact_start_tag_latent = this.toNode_exact_start_tag_latent(bIndex, tag, latent);
						long node_incomplete_start_tag_latent = this.toNode_incomplete_start_tag_latent(bIndex, tag, latent);
						
						if(should_start){
//							System.err.println("started.."+(N+bIndex));
							network.addNode(node_incomplete_start_tag_latent);
							network.addEdge(node_exact_start_tag_latent, new long[]{node_incomplete_start_tag_latent});
						} else {
							network.addEdge(node_exact_start_tag_latent, new long[]{node_terminate});
						}
						
						if(should_terminate){
							network.addNode(node_incomplete_start_tag_latent);
							long node_incomplete_start_tag_latent_next = this.toNode_incomplete_start_tag_latent(bIndex+1, tag, latent);
							if(should_connect_next){
//								System.err.println("terminated+connected.."+(N+bIndex+1));
								network.addEdge(node_incomplete_start_tag_latent, new long[]{node_terminate, node_incomplete_start_tag_latent_next});
							} else {
//								System.err.println("terminated.."+(N+bIndex+1));
								network.addEdge(node_incomplete_start_tag_latent, new long[]{node_terminate});
							}
						}
...
//							System.err.println("connected.."+(N+bIndex));
							network.addNode(node_incomplete_start_tag_latent);
							long node_incomplete_start_tag_latent_next = this.toNode_incomplete_start_tag_latent(bIndex+1, tag, latent);
							network.addEdge(node_incomplete_start_tag_latent, new long[]{node_incomplete_start_tag_latent_next});
						}
					}
	...
	public void build(IELinearLatentNetwork network, int bIndex){
		...
		for(int k = 0; k<tags.length; k++){
			...
			for(int latent = 0; latent<numLatent; latent++){
				...
				if(bIndex<-1){
					long node_incomplete_start_tag_latent_next = this.toNode_incomplete_start_tag_latent(bIndex+1, tag, latent);
					if(network.contains(node_incomplete_start_tag_latent_next)){
						network.addEdge(node_incomplete_start_tag_latent, new long[]{node_incomplete_start_tag_latent_next});
						network.addEdge(node_incomplete_start_tag_latent, new long[]{node_terminate, node_incomplete_start_tag_latent_next});
					}
				}
...
//		System.err.println("size="+nodes_exact_start_tags.length);
	...
	@Override
	public IELinearLatentInstance decompile(Network network) {
		
		IELinearLatentNetwork iNetwork = (IELinearLatentNetwork)network;
		IELinearLatentInstance inst = (IELinearLatentInstance)network.getInstance();
		IELinearLatentInstance inst_output = (IELinearLatentInstance) inst.duplicate();
	...
	private void decode(IELinearLatentNetwork network, int node_k, UnlabeledTextSpan span){
		...
		if(hybridType == IELinearLatentConfig.NODE_TYPE.EXACT_START_TAG_LATENT.ordinal())
		{
			...
			if(hybridType_child == IELinearLatentConfig.NODE_TYPE.TERMINATE.ordinal()){
				//DO NOTHING..
			}
			
			else if(hybridType_child == IELinearLatentConfig.NODE_TYPE.INCOMPLETE_START_TAG_LATENT.ordinal()){
				int start_index = N - array[0];
				int tag_id = array[2]-1;
...
//					span.label_predict(start_index, end_index, new Mention(start_index, end_index, end_index-1, end_index, tag));
	...
	private ArrayList<Integer> findBoundaries(int N, IELinearLatentNetwork network, int node_k){
		ArrayList<Integer> results = new ArrayList<Integer>();
		this.findBoundariesHelper(N, results, network, node_k);
		return results;
	}
	
	private void findBoundariesHelper(int N, ArrayList<Integer> results, IELinearLatentNetwork network, int node_k){
		long node = network.getNode(node_k);
		int[] array = NetworkIDMapper.toHybridNodeArray(node);
//		int hybridType = array[4];
...
//				System.err.println(">>>\t"+hybridType+"\t"+IELinearConfig.NODE_TYPE.INCOMPLETE_START_TAG.ordinal()+"\t"+array[0]+"\t"+end_index+"\t"+N);
	...
	private long toNode_after_start(int bIndex){
		...
		int srcWidth = IELinearLatentConfig._MAX_SENT_LENGTH;
		int tgtHeight = this._tags.length + 3;
		int tgtWidth = 0;
		int hybridType = IELinearLatentConfig.NODE_TYPE.AFTER_START.ordinal();
	...
	private long toNode_exact_start(int bIndex){
		...
		int tgtHeight = this._tags.length + 3;
		int tgtWidth = 0;
		int hybridType = IELinearLatentConfig.NODE_TYPE.EXACT_START.ordinal();
		return NetworkIDMapper.toHybridNodeID(new int[]{srcHeight, srcWidth, tgtHeight, tgtWidth, hybridType});
	}
	...
	private long toNode_exact_start_tag(int bIndex, SemanticTag tag){
		int srcHeight = - bIndex;
		int srcWidth = IELinearLatentConfig._MAX_SENT_LENGTH;
		int tgtHeight = tag.getId() + 1;
		int tgtWidth = 1000;
		int hybridType = IELinearLatentConfig.NODE_TYPE.EXACT_START_TAG.ordinal();
		return NetworkIDMapper.toHybridNodeID(new int[]{srcHeight, srcWidth, tgtHeight, tgtWidth, hybridType});
	}
	
	private long toNode_exact_start_tag_latent(int bIndex, SemanticTag tag, int latent_id){
		int srcHeight = - bIndex;
		int srcWidth = IELinearLatentConfig._MAX_SENT_LENGTH;
		int tgtHeight = tag.getId() + 1;
		int tgtWidth = latent_id;
		int hybridType = IELinearLatentConfig.NODE_TYPE.EXACT_START_TAG_LATENT.ordinal();
		return NetworkIDMapper.toHybridNodeID(new int[]{srcHeight, srcWidth, tgtHeight, tgtWidth, hybridType});
	}
	
	private long toNode_incomplete_start_tag_latent(int bIndex, SemanticTag tag, int latent_id){
//		if(bIndex==-2){
...
//		}
		...
		int tgtHeight = tag.getId() + 1;
		int tgtWidth = latent_id;
		int hybridType = IELinearLatentConfig.NODE_TYPE.INCOMPLETE_START_TAG_LATENT.ordinal();
		return NetworkIDMapper.toHybridNodeID(new int[]{srcHeight, srcWidth, tgtHeight, tgtWidth, hybridType});
	}
	...
	private long toNode_root(int bIndex){
		int srcHeight = - bIndex;
		int srcWidth = IELinearLatentConfig._MAX_SENT_LENGTH;
		int tgtHeight = this._tags.length + 4;
		int tgtWidth = 0;
		int hybridType = IELinearLatentConfig.NODE_TYPE.ROOT.ordinal();
		return NetworkIDMapper.toHybridNodeID(new int[]{srcHeight, srcWidth, tgtHeight, tgtWidth, hybridType});
	}
	...
	private long toNode_terminate(){
		...
		int tgtHeight = 0;
		int tgtWidth = 0;
		int hybridType = IELinearLatentConfig.NODE_TYPE.TERMINATE.ordinal();
		return NetworkIDMapper.toHybridNodeID(new int[]{srcHeight, srcWidth, tgtHeight, tgtWidth, hybridType});
	}
