...
public class IELinearLatentFeatureManager extends FeatureManager{
...
//	private TextSpan _span;
	...
	public IELinearLatentFeatureManager(GlobalNetworkParam param) {
		super(param);
...
//		System.err.println("cache disabled.");
	...
	private FeatureArray extract(IELinearLatentInstance inst, FEATURE_TYPE ft, int word_id, int tag_id, int latent_id, FeatureArray fa){
		...
		if(this._cache != null){
			...
			if(this._cache[inst_id-1][tag_id-1][latent_id][ft_id]==null){
				this._cache[inst_id-1][tag_id-1][latent_id][ft_id] = new int[inst.getInput().length()][];
			}
			
			if(this._cache[inst_id-1][tag_id-1][latent_id][ft_id][word_id]!=null){
				int[] fs = this._cache[inst_id-1][tag_id-1][latent_id][ft_id][word_id];
				return new FeatureArray(fs, fa);
			}
		}
		
		int[] fs = this.extract_helper(inst, ft, word_id, tag_id, latent_id);
		...
		if(this._cache!=null){
			...
			this._cache[inst_id-1][tag_id-1][latent_id][ft_id][word_id] = fs;
		}
		
		return new FeatureArray(fs, fa);
	}
	
	private int[] extract_helper(IELinearLatentInstance inst, FEATURE_TYPE ft, int word_id, int tag_id, int latent_id){
...
//				if(att.equals("POS") || att.equals("curr"))
				{
					f[k++] = this._param_g.toFeature(ft.name(), tag_id+":"+latent_id+":"+att, val);
...
//		System.exit(1);
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k) {
		...
		if(node_parent_type == IELinearLatentConfig.NODE_TYPE.ROOT.ordinal()){
			return FeatureArray.EMPTY;
		}
		if(node_parent_type == IELinearLatentConfig.NODE_TYPE.TERMINATE.ordinal()){
			return FeatureArray.EMPTY;
		}
		if(node_parent_type == IELinearLatentConfig.NODE_TYPE.AFTER_START.ordinal()){
			return FeatureArray.EMPTY;
		}
		if(node_parent_type == IELinearLatentConfig.NODE_TYPE.EXACT_START.ordinal()){
			return FeatureArray.EMPTY;
		}
		
		if(node_parent_type == IELinearLatentConfig.NODE_TYPE.EXACT_START_TAG_LATENT.ordinal()){
			...
			if(node_child_type == IELinearLatentConfig.NODE_TYPE.TERMINATE.ordinal()){
				return FeatureArray.EMPTY;
...
//				return fa;
			...
			if(node_child_type == IELinearLatentConfig.NODE_TYPE.INCOMPLETE_START_TAG_LATENT.ordinal()){
				int bIndex = - node_child_arr[0];
				int tag_id = node_child_arr[2]-1;
				int latent_id = node_parent_arr[3];
				UnlabeledTextSpan span = inst.getInput();//.getSpan();
				int word_id = span.length() + bIndex;
				
				FeatureArray fa = FeatureArray.EMPTY;
				fa = this.extract(inst, FEATURE_TYPE.START_MENTION, word_id, tag_id, latent_id, fa);
		...
		if(node_parent_type == IELinearLatentConfig.NODE_TYPE.INCOMPLETE_START_TAG_LATENT.ordinal()){
			int bIndex = - node_parent_arr[0];
			int tag_id = node_parent_arr[2]-1;
			int latent_id = node_parent_arr[3];
			UnlabeledTextSpan span = inst.getInput();//.getSpan();
			int word_id = span.length() + bIndex;
			
//			System.err.println("xx:"+node_parent_arr[0]);

			FeatureArray fa = FeatureArray.EMPTY;
			fa = this.extract(inst, FEATURE_TYPE.WITHIN_MENTION, word_id, tag_id, latent_id, fa);
			...
			for(int i = 0; i<children_k.length; i++){
				...
				if(node_child_type == IELinearLatentConfig.NODE_TYPE.TERMINATE.ordinal()){
					fa = this.extract(inst, FEATURE_TYPE.END_MENTION, word_id, tag_id, latent_id, fa);
				}
		...
		if(node_parent_type == IELinearLatentConfig.NODE_TYPE.EXACT_START_TAG.ordinal()){
			return FeatureArray.EMPTY;
		}
...
//		}
		...
		throw new RuntimeException("This should not happen."+node_parent_type);
