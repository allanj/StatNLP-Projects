...
public class DataReader_Standard2CRFPP_BLIOU_CC {
			...
			for(String subfolder : new String[]{"English"}){
				for(String set : new String[]{"train", "test", "dev"}){
					...
					for(String overlap : new String[]{"cascaded"}){
						...
						for(int tag_index = 0; tag_index<=all_tags.size(); tag_index++){
	...
	public static void process_cascaded(Scanner scan, PrintWriter p, IEManager manager, int tag_index) throws FileNotFoundException{
		...
		while(scan.hasNextLine()){
			...
			p.println(span.toCRFPP_format_withType_BLIOU(all_tags, tag_index));
			p.flush();
