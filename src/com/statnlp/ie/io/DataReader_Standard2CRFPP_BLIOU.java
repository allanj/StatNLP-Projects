...
public class DataReader_Standard2CRFPP_BLIOU {
			...
			for(String subfolder : new String[]{"English"}){
				for(String set : new String[]{"train", "test", "dev"}){
					...
					for(String overlap : new String[]{"no_overlap"}){
						...
						String filename_output = "experiments/mention/lcrf-bliou/"+IEConfig._type.name()+"/"+folder+"/"+subfolder+"/"+overlap+"/"+set+".data";
...
//		IEManager manager = EventExtractionReader.readIEManager(filename_template);
//		
//		for(String folder : new String[]{"ace2004_normalized", "ace2005_normalized"}){
//			for(String subfolder : new String[]{"nw", "bc"}){
...
//						f.mkdirs();
//						
//						String filename_input = "data/mention-standard/"+IEConfig._type.name()+"/"+folder+"/"+subfolder+"/"+set+".data";
//						String filename_output = "experiments/mention/lcrf-bliou/"+IEConfig._type.name()+"/"+folder+"/"+subfolder+"/"+overlap+"/"+set+".data";
//						
//						Scanner scan = new Scanner(new File(filename_input));
//						PrintWriter p = new PrintWriter(new File(filename_output));
...
//	
	...
	public static void process_noOverlapping(Scanner scan, PrintWriter p, IEManager manager) throws FileNotFoundException{
		...
		while(scan.hasNextLine()){
			...
			p.println(span.toCRFPP_format_withType_BLIOU());
			p.flush();
