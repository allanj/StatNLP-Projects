...
public class SVM {
	...
	private static void solve_c_svc(SVMProblem prob, SVMParam param, double[] alpha, Solver.SolutionInfo si, double Cp, double Cn) {
	...
	private static void solve_nu_svc(SVMProblem prob, SVMParam param, double[] alpha, Solver.SolutionInfo si) {
	...
	private static void solve_one_class(SVMProblem prob, SVMParam param, double[] alpha, Solver.SolutionInfo si) {
	...
	private static void solve_epsilon_svr(SVMProblem prob, SVMParam param, double[] alpha, Solver.SolutionInfo si) {
	...
	private static void solve_nu_svr(SVMProblem prob, SVMParam param, double[] alpha, Solver.SolutionInfo si) {
	...
	static decision_function svm_train_one(SVMProblem prob, SVMParam param, double Cp, double Cn) {
	...
	private static void svm_binary_svc_probability(SVMProblem prob, SVMParam param, double Cp, double Cn, double[] probAB) {
	...
	private static double svm_svr_probability(SVMProblem prob, SVMParam param) {
	...
	public static SVMModel svm_train(SVMProblem prob, SVMParam param) {
		...
		if (param.svm_type == SVMParam.ONE_CLASS
			...
			if (param.probability == 1 && (param.svm_type == SVMParam.EPSILON_SVR || param.svm_type == SVMParam.NU_SVR)) {
	...
	public static void svm_cross_validation(SVMProblem prob, SVMParam param, int nr_fold, double[] target) {
		...
		for (i = 0; i < nr_fold; i++) {
			...
			if (param.probability == 1
					&& (param.svm_type == SVMParam.C_SVC || param.svm_type == SVMParam.NU_SVC)) {
	...
	public static double svm_get_svr_probability(SVMModel model) {
		...
		if ((model.param.svm_type == SVMParam.EPSILON_SVR || model.param.svm_type == SVMParam.NU_SVR)
				&& model.probA != null)
	...
	public static double svm_predict_values(SVMModel model, SVMNode[] x, double[] dec_values) {
		...
		int i;
		if (model.param.svm_type == SVMParam.ONE_CLASS
				|| model.param.svm_type == SVMParam.EPSILON_SVR
				|| model.param.svm_type == SVMParam.NU_SVR) {
	...
	public static double svm_predict_probability(SVMModel model, SVMNode[] x, double[] prob_estimates) {
		...
		if ((model.param.svm_type == SVMParam.C_SVC || model.param.svm_type == SVMParam.NU_SVC)
	...
	public static String svm_check_parameter(SVMProblem prob, SVMParam param) {
		...
		if (svm_type != SVMParam.C_SVC && svm_type != SVMParam.NU_SVC
				&& svm_type != SVMParam.ONE_CLASS
				&& svm_type != SVMParam.EPSILON_SVR
				&& svm_type != SVMParam.NU_SVR)
			return "unknown svm type";
		
		// kernel_type, degree
		
		int kernel_type = param.kernel_type;
		if (kernel_type != SVMParam.LINEAR
				&& kernel_type != SVMParam.POLY
				&& kernel_type != SVMParam.RBF
				&& kernel_type != SVMParam.SIGMOID
				&& kernel_type != SVMParam.PRECOMPUTED)
		...
		if (param.probability == 1 && svm_type == SVMParam.ONE_CLASS)
	...
	public static int svm_check_probability_model(SVMModel model) {
		if (((model.param.svm_type == SVMParam.C_SVC || model.param.svm_type == SVMParam.NU_SVC)
				&& model.probA != null && model.probB != null)
				|| ((model.param.svm_type == SVMParam.EPSILON_SVR || model.param.svm_type == SVMParam.NU_SVR) && model.probA != null))
			return 1;
		else
			return 0;
	}
