...
/**
 * the trainer for preference modeling.
 * @author luwei
 * @version 1.0
 */

public class PreferenceTrainer {
	...
	//the topK value used for soft constraint learning. In the paper we set it to 10.
	...
	public static void main(String args[])throws Exception{
		...
		while(scan.hasNextLine()){
			...
			} else if(type.equals("preferenceFileName")){
				preferenceFileName = arg;
			}
		}
		
		Manager manager = toManager(eventSpanFileName_train, eventStructureFileName);
		FeatureManager fm = new FeatureManager();
		PreferenceManager cm = PreferenceManager.readPreferences(preferenceFileName, manager);
		...
		opt(manager, fm, cm, trainSpans, event, allfeatures);
		
		System.err.println("Writing the learned model into the disk.");
		ObjectOutputStream out;
		out = new ObjectOutputStream(new FileOutputStream("data/model/manager-"+eventName+".obj"));
		out.writeObject(manager);
		out.flush();
		out.close();
		out = new ObjectOutputStream(new FileOutputStream("data/model/fm-"+eventName+".obj"));
		out.writeObject(fm);
		out.flush();
		out.close();
